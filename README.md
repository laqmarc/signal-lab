...
5. Prem **CHECK**. Rebràs una puntuació de 0 a 100% i una pista sobre què has d'ajustar. El 100% completa el nivell.
...
- Torna a prémer el botó de reproducció per aturar el so. Una nova reproducció atura l'anterior. Editar el patch atura la nota del jugador perquè el so no continuï representant paràmetres antics.
...
L'estat `Patch` conté els paràmetres i una llista de connexions entre ports. La UI el modifica; el motor rep una còpia per cada reproducció; la puntuació el compara amb l'objectiu immutable del nivell. No s'inclou cap mòdul addicional.
...
- `npm test`: 9 proves amb el runner integrat de Node. Comproven connexions invàlides, desconnexió, aïllament de l'objectiu, puntuació, límits i que les pistes permeten arribar a la solució.
- Amb `npm run dev` actiu, obre `/tests/audio.html` i prem **Executa les proves**: 8 proves amb `OfflineAudioContext` i el motor real, sense emetre àudio. Comproven mostres idèntiques per al mateix patch, silenci quan falten cables, diferències entre formes d'ona, efecte dels tres controls, absència de saturació en els casos límit a 44,1/48 kHz i final de nota.
